...
t_player		*parse_champion(char *filename, int id)
{
	...
	champ = init_player(id);
	if ((fd = open(filename, O_RDONLY)) < 0)
		terminate(ERR_OPEN_FILE);
	if (parse_int32(fd) != COREWAR_EXEC_MAGIC)
		terminate(ERR_INVALID_MAGIC);
	champ->name = parse_str(fd, PROG_NAME_LENGTH);
	if (parse_int32(fd) != 0)
		terminate(ERR_NO_NULL);
	if ((champ->code_size = parse_int32(fd)) < 0 ||
		champ->code_size > CHAMP_MAX_SIZE)
		terminate(ERR_INVALID_CODE_SIZE);
	champ->comment = parse_str(fd, COMMENT_LENGTH);
	if (parse_int32(fd) != 0)
		terminate(ERR_NO_NULL);
	champ->code = parse_code(fd, (size_t)champ->code_size);
	return (champ);
}
