...
static t_bool	is_register(t_vm *vm, int32_t next_op, int32_t step)
{
	int8_t r_id;

	r_id = get_byte(vm, next_op, step);
...
t_bool			is_args_valid(t_cursor *cursor, t_vm *vm, t_op *op)
{
	...
	while (i < op->args_num)
	{
		if ((cursor->args[i] == T_REG)
			&& !is_register(vm, cursor->next_op, step))
			return (false);
		step += step_size(cursor->args[i], op);
		i++;
	}
	return (true);
}
