...
void			draw_lives_bar(t_vm *vm, t_bool current)
{
	...
	while (i < vm->amount_of_champs)
		if (current)
			sum_lives += vm->champs[i++]->current_lives_num;
		else
			sum_lives += vm->champs[i++]->previous_lives_num;
	i = 0;
	pos = 0;
	if (!sum_lives)
		draw_empty_bar(vm);
		...
		while (i < vm->amount_of_champs)
		{
			len = calc_bar_length((current) ? vm->champs[i]->current_lives_num
						: vm->champs[i]->previous_lives_num, sum_lives);
